import CommonButton from "inputComponent/CommonButton";
import CommonTextInput from "inputComponent/CommonTextInput";
import { useState, useContext } from "react";
import FeedbackReferenceContext from "context/FeedbackReferenceContext";

function References() {
  const {
    addReference,
    ReferenceFeedback,
    ButtonFeedBackHandler,
    removeLayoutHandler,
  } = useContext(FeedbackReferenceContext);

  // const ButtonFeedBackHandler = (status) => {
  //   console.log(status);
  // };
  return (
    <div>
      <div class="text-onSurface">
        <h6 class="text-light mb-3 text-center text-xl font-semibold uppercase md:text-2xl">
          Reference
        </h6>

        <div class="w-full rounded-lg border border-dashed p-2 text-onSurface">
          {addReference.map((post, index) => (
            <>
              <div class="flex justify-end">
                {addReference.length !== 1 ? (
                  <button
                    class="cursor-pointer text-primary hover:text-onPrimary"
                    onClick={() => {
                      removeLayoutHandler(post, index);
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke-width="1.5"
                      stroke="currentColor"
                      class="h-6 w-6"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                      />
                    </svg>
                  </button>
                ) : (
                  ""
                )}
              </div>
              <div class="my-4 grid w-full grid-cols-1 md:grid-cols-3 md:gap-4">
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="text"
                    label="Name"
                    name=""
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="text"
                    label="Position"
                    name="position"
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="text"
                    label="Organization"
                    name="organizationName"
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="text"
                    label="Mailing Address"
                    name="mailingAddress"
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="number"
                    label="Mobile"
                    name="mobileNo"
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
                <div class="grid grid-cols-1">
                  <CommonTextInput
                    inputType="email"
                    label="Email Address"
                    name="email"
                    InputFeedBack={(name, value) => {
                      ReferenceFeedback(value, index, name);
                    }}
                  />
                </div>
              </div>
            </>
          ))}
        </div>
      </div>

      <div class="flex justify-end gap-4 p-4">
        <CommonButton
          text="Add Reference"
          ButtonFeedBack={ButtonFeedBackHandler}
        />
      </div>
    </div>
  );
}

export default References;
